import scrapy


class MediaSpiderSpider(scrapy.Spider):
    name = 'media_spider'
    allowed_domains = ['google.com/search?q=gilan+holding', 'google.com/search?q=socar' , 'google.com/search?q=carçıoğlu+group' ]
    start_urls = ['http://google.com/search?q=gilan+holding',
		  'http://google.com/search?q=socar',
		 'http://google.com/search?q=carçıoğlu+group']

    def parse(self, response):

        response.selector.remove_namespaces()

        #Extract article information
        titles = response.xpath('//div[@class="BNeawe vvjwJb AP7Wnd"]/text()').extract()
        headers = response.xpath('//div[@class = "BNeawe s3v9rd AP7Wnd"]/text()').extract()
        links = response.xpath('//div[@class = "kCrYT"]/a/@href').extract()

        for item in zip(titles,headers, links):
            scraped_info = {
                'title' : item[0],
		'header' : item[1],
		'link' : item[2]
            }

            yield scraped_info

# get() <=> extract_first()
# getall() <=> extract()



#response.xpath('//div[@class="BNeawe vvjwJb AP7Wnd"]/text()').extract()    -- title (with xpath)
# response.css('div[class="BNeawe vvjwJb AP7Wnd"]::text').extract()  -- title (with css selector)

# response.xpath('//div[@class = "kCrYT"]/a/@href').extract_first() -- link (with xpath)
# response.css('div[class="kCrYT"] a::attr(href)').extract()  -- link (with css selector)

# response.xpath('//div[@class = "BNeawe s3v9rd AP7Wnd"]/text()').extract() -- header (with xpath)
# response.css('div[class="BNeawe s3v9rd AP7Wnd"]::text').extract()  -- header (with css selector)

